...
#1读取用例
excelDir = PATH(config_path + r'\小程序接口测试用例.xlsx')
#打开excel
...
token = '2McNsXJ7ckiP1JJfitC0RRRzEADHEQx8D9QM'
for one in range(1,workSheet.nrows):
    ...
    expectCode = expectRes['status']#预计结果

    # c_data = {cellData}
    c_data = json.loads(cellData)
    if (one == 10):
        c_data['bizId'] = getbizId(32)
        c_data['startTime'] = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
        print(c_data)
...
workbookNew.save(PATH(config_path + r'\小程序接口测试结果.xlsx'))